...
<?php
  ...
  if(isset($_POST["submit"])){
    ...
    $qt=$_POST["qty"];
      $time = date('H:i:s');
    $sql="INSERT into cafe values('cafe','$myUs','$rm','$tp','$qt','$time',NULL);";
  ...
  $coffee=array("Decaf","Cappucino","Iced Latte","Espresso");
  $room=[];
...
<div id="container">
    <div id="book_taxi">
          <img src="hotel/cafe2.jpg" alt="Company's Logo" style="width:100%;height:100%;opacity: 0.3">
          
          <div id="book_table" style="width: 90%;height: 500px;position: relative;top: -785px;left:100px;">
              <img src="hotel/cafe1.png" alt="Company's Logo" style="position: relative;top:-80px;left:250px;">
              <img src="hotel/coffee1.png" alt="Company's Logo" style="height: 200px;width:200px;position: relative;top:0px;left:-540px">
              <form method="post" action="./cafe.php">
                <a>ROOM NUMBER: </a> <select name="room">
                                <?php
                                  foreach($room as $r){ ?>
                ...
                <a style="position: relative;left: 30px">COFFEE:</a> <select name="type" style="position: relative;left: 30px">
                                <?php
                                  foreach($coffee as $r ) { ?>
                                          <option value="<?php echo $r ?>"> <?php echo $r ?></option>
                                        <?php
                        ...
                        <a style="position: relative;left: 70px">QUANTITY:</a> <select name="qty" style="position: relative;left: 70px;width: 30px">
                                <?php
                  ...
                  <input type="submit" name="submit" value="" style="background-image: url('./hotel/booknow.jpg');background-size:100% 100%;position: relative;top: 30px;left:750px;height: 60px;width: 250px">
              </form>
              
          </div>
          <div id="current_booking" style="width: 80%;height: 100px;position: relative;top:-980px;left: 100px">
              <?php
                $sql="SELECT room_no,type,quantity,time_book from cafe where book_id=$myUs;";
                $qw=mysqli_query($con,$sql);
              if(mysqli_affected_rows($con)>0){
                echo "<h1>YOUR CURRENT BOOKINGS:<br><br>";
                echo "<table border=1 style='width:700px'><tr><th><a>ROOM NUMBER</a></th><th>TYPE</th><th>QUANTITY</th><th>TIME</th></tr>";
                while($r=mysqli_fetch_row($qw)){
                    echo "<tr><th>".strtoupper($r[0])."</th><th>".strtoupper($r[1])."</th><th>$r[2]</th><th>$r[3]</th></tr>";
                }
                echo '</table></h1>';
...
<div id="rest_menu" style="background-color: black;height: 70px;width: 80.7%;position: fixed;top:60px;left:278px">
  <div id="rest_option" style="position: relative;top:30px;left:50px">
      ...
      <a href="bar.php" >BAR NINETEEN 12</a>
      <a href="pool.php" >POOL LOUNGE</a>
      <a href="cafe.php" style="color: yellow">CABANA COFFEE</a>
  </div>
</div>
</body>
</html>